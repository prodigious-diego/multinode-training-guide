...
def _(x: Tensor, w: Tensor, *_):
    ...
    assert x.is_contiguous() and w.is_contiguous()
    result = (x @ w.T).contiguous()
    return result, x.to(torch.float8_e4m3fn), w.to(torch.float8_e4m3fn)
...
def cached_fineweb(num_chunks=8):
    import os
    ...
    # full fineweb10B is 103 chunks. Each chunk is 100M tokens
...
if master_process:
    print("Downloading fineweb10B from huggingface if it is not already downloaded")
    cached_fineweb()
...
if master_process:
    ...
    logfile = f"/data/logs/{run_id}.txt"
    print(logfile)
...
print("Warmup the training kernels, then re-initialize the state so we aren't cheating")
warmup_steps = 10
initial_state = dict(
